...
let parse s =
	Lexing.from_string s |> TagParser.parsed TagLexer.token ;;
...
let main () =
	let specs =
	[("-i", Arg.Set_string infile,"Interpret mode. Usage: -i <infile>")] in
